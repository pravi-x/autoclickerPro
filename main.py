...
class AutoClickerApp(tk.Tk):
    def __init__(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Auto Clicker Pro", style="Title.TLabel")
        ...
        # Start info updates and default hotkey
        self.update_info()
    ...
    def setup_styles(self):
        ...
        # Frame styles
        style.configure("Section.TFrame", background="#34495e", relief="raised", borderwidth=2)
        ...
        style.map("Action.TButton",
          background=[("active", "#5dade2"), ("pressed", "#2e86c1"), ("disabled", "#95a5a6")],
          foreground=[("disabled", "#ecf0f1")])
        style.map("Start.TButton",
          background=[("active", "#58d68d"), ("pressed", "#229954"), ("disabled", "#95a5a6")],
          foreground=[("disabled", "#ecf0f1")])
        style.map("Stop.TButton",
          background=[("active", "#e74c3c"), ("pressed", "#943126"), ("disabled", "#95a5a6")],
          foreground=[("disabled", "#ecf0f1")])
    ...
    def create_mouse_info_section(self, parent):
        ...
        ttk.Label(info_frame, text="📍 Live Mouse Information", style="Section.TLabel").pack(anchor="w")
        ...
        ttk.Label(info_content, textvariable=self.position_var, style="Regular.TLabel").pack(side=tk.LEFT, padx=10)
        ttk.Label(info_content, textvariable=self.color_var, style="Regular.TLabel").pack(side=tk.LEFT, padx=10)

    def create_input_section(self, parent):
        input_frame = ttk.Frame(parent, style="Section.TFrame", padding="10")
        input_frame.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Label(input_frame, text="⚙️ Action Configuration", style="Section.TLabel").pack(anchor="w")
        ...
        # Labels
        ttk.Label(grid_frame, text="Action Name", style="Input.TLabel").grid(row=0, column=0, sticky="w", padx=5)
        ttk.Label(grid_frame, text="Click Position", style="Input.TLabel").grid(row=0, column=1, sticky="w", padx=5)
        ttk.Label(grid_frame, text="Delay (seconds)", style="Input.TLabel").grid(row=0, column=2, sticky="w", padx=5)
        ttk.Label(grid_frame, text="Monitor Position", style="Input.TLabel").grid(row=0, column=3, sticky="w", padx=5)
        ttk.Label(grid_frame, text="Target Color", style="Input.TLabel").grid(row=0, column=4, sticky="w", padx=5)
        ...
        self.click_pos_input.grid(row=1, column=1, sticky="ew", padx=5, pady=(3, 0))
        
        self.delay_input = ttk.Entry(grid_frame, font=("Arial", 9))
        self.delay_input.grid(row=1, column=2, sticky="ew", padx=5, pady=(3, 0))
        
        self.monitor_pos_input = ttk.Entry(grid_frame, font=("Arial", 9))
        self.monitor_pos_input.grid(row=1, column=3, sticky="ew", padx=5, pady=(3, 0))
        
        self.target_color_input = ttk.Entry(grid_frame, font=("Arial", 9))
        self.target_color_input.grid(row=1, column=4, sticky="ew", padx=5, pady=(3, 0))
        
        # Placeholder text examples
        ttk.Label(grid_frame, text="e.g. 'Button Click'", style="Regular.TLabel").grid(row=2, column=0, sticky="w", padx=5, pady=(2, 0))
        ttk.Label(grid_frame, text="e.g. '100,200'", style="Regular.TLabel").grid(row=2, column=1, sticky="w", padx=5, pady=(2, 0))
        ttk.Label(grid_frame, text="e.g. '1.5'", style="Regular.TLabel").grid(row=2, column=2, sticky="w", padx=5, pady=(2, 0))
        ttk.Label(grid_frame, text="e.g. '150,300' (opt)", style="Regular.TLabel").grid(row=2, column=3, sticky="w", padx=5, pady=(2, 0))
        ttk.Label(grid_frame, text="e.g. '255,0,0' (opt)", style="Regular.TLabel").grid(row=2, column=4, sticky="w", padx=5, pady=(2, 0))
        
        # Add action button
        self.add_action_button = ttk.Button(input_frame, text="➕ Add Action", 
                                          command=self.add_action, style="Action.TButton")
    ...
    def create_action_list_section(self, parent):
        ...
        ttk.Label(list_frame, text="📋 Action Sequence", style="Section.TLabel").pack(anchor="w")
        ...
        self.action_list = tk.Listbox(list_content, 
                                    height=6, 
                                    bg="#34495e", 
                                    fg="#ecf0f1", 
        ...
        self.scrollbar = ttk.Scrollbar(list_content, orient=tk.VERTICAL, command=self.action_list.yview)
        self.action_list.config(yscrollcommand=self.scrollbar.set)
    ...
    def create_monitor_section(self, parent):
        ...
        ttk.Label(monitor_frame, text="📺 Action Monitor", style="Section.TLabel").pack(anchor="w")
        
        monitor_content = ttk.Frame(monitor_frame, style="Input.TFrame")
        monitor_content.pack(fill=tk.BOTH, expand=True, pady=(5, 0))
        
        self.monitor_text = tk.Text(monitor_content, 
                                  height=6, 
                                  wrap=tk.WORD, 
                                  state=tk.DISABLED,
                                  bg="#2c3e50",
    ...
    def create_control_section(self, parent):
        ...
        ttk.Label(control_frame, text="🎮 Controls", style="Section.TLabel").pack(anchor="w")
        
        # Repeat count
        repeat_frame = ttk.Frame(control_frame, style="Input.TFrame")
        repeat_frame.pack(fill=tk.X, pady=(10, 10))
        
        ttk.Label(repeat_frame, text="Repeat Count (0 for infinite):", style="Input.TLabel").pack(side=tk.LEFT, padx=5)
        ...
        self.save_button = ttk.Button(button_frame, text="💾 Save", 
                                    command=self.save_actions, style="Action.TButton")
        self.load_button = ttk.Button(button_frame, text="📂 Load", 
                                    command=self.load_actions, style="Action.TButton")
        self.start_button = ttk.Button(button_frame, text="▶️ Start", 
                                     command=self.start_sequence, style="Start.TButton")
        self.stop_button = ttk.Button(button_frame, text="⏹️ Stop", 
                                    command=self.stop_sequence, style="Stop.TButton", state='disabled')
    ...
    def create_hotkey_section(self, parent):
        hotkey_frame = ttk.Frame(parent, style="Section.TFrame", padding="10")
        hotkey_frame.pack(fill=tk.X)
        
        ttk.Label(hotkey_frame, text="⌨️ Global Hotkey", style="Section.TLabel").pack(anchor="w")
        
        hotkey_content = ttk.Frame(hotkey_frame, style="Input.TFrame")
        hotkey_content.pack(fill=tk.X, pady=(10, 0))
        
        ttk.Label(hotkey_content, text="Hotkey Combination:", style="Input.TLabel").pack(side=tk.LEFT, padx=5)
        self.hotkey_input = ttk.Entry(hotkey_content, font=("Arial", 9), width=15)
        self.hotkey_input.pack(side=tk.LEFT, padx=5)
        self.hotkey_input.insert(0, "ctrl+alt+1")
        
        self.set_hotkey_button = ttk.Button(hotkey_content, text="🔧 Set Hotkey", 
                                          command=self.register_hotkey, style="Action.TButton")
        self.set_hotkey_button.pack(side=tk.LEFT, padx=5)
    ...
    def add_action(self):
        ...
        if click_coords is None:
            error_message += "• Click position must be x,y with numeric values.\n"
        ...
        if monitor_coords and target_color:
            action_text = f"{name_text} - Click {click_coords}, monitor {monitor_coords} for color {target_color}, delay {delay_time}s"
        elif monitor_coords:
            action_text = f"{name_text} - Click {click_coords}, monitor {monitor_coords} for any color change, delay {delay_time}s"
        else:
            action_text = f"{name_text} - Click {click_coords}, no monitoring, delay {delay_time}s"

        self.action_list.insert(tk.END, action_text)
        self.actions.append((name_text, click_coords, monitor_coords, target_color, delay_time))
    ...
    def save_actions(self):
        ...
        try:
            with open(path, mode='w', newline='') as file:
                ...
                for name, click, monitor, color, delay in self.actions:
                    row = [name, click[0], click[1]]
            ...
            with open(path, mode='r') as file:
                ...
                for row in reader:
                    if len(row) >= 3:
                        ...
                        click_coords = (int(row[1]), int(row[2]))
                        monitor_coords = None
                        target_color = None
                        delay_time = 0.5
                        if len(row) >= 5 and row[3] and row[4]:
                            monitor_coords = (int(row[3]), int(row[4]))
                        if len(row) >= 8 and row[5] and row[6] and row[7]:
                            target_color = (int(row[5]), int(row[6]), int(row[7]))
                        if len(row) >= 9:
                            delay_time = float(row[8])
                        if monitor_coords and target_color:
                            action_text = f"{name} - Click {click_coords}, monitor {monitor_coords} for color {target_color}, delay {delay_time}s"
                        elif monitor_coords:
                            action_text = f"{name} - Click {click_coords}, monitor {monitor_coords} for any color change, delay {delay_time}s"
                        else:
                            action_text = f"{name} - Click {click_coords}, no monitoring, delay {delay_time}s"
                        self.actions.append((name, click_coords, monitor_coords, target_color, delay_time))
                        self.action_list.insert(tk.END, action_text)
    ...
    def start_sequence(self):
        ...
        try:
            ...
            if repeat_count < 0:
                messagebox.showwarning("Input Error", "Repeat count must be non-negative.")
                return
        except ValueError:
        ...
        self.thread = threading.Thread(target=self.run_actions, args=(repeat_count,), daemon=True)
        self.thread.start()
    ...
    def execute_actions(self):
        for i, (name, click_pos, monitor_pos, target_color, delay_time) in enumerate(self.actions, 1):
            if not self.running:
                break
                
            # Log the action being performed
            self.log_to_monitor(f"🖱️ Action {i}: {name} - Clicking ({click_pos[0]}, {click_pos[1]})")
            ...
            if monitor_pos:
                if target_color:
                    ...
                    self.log_to_monitor(f"👁️ Monitoring pixel ({monitor_pos[0]}, {monitor_pos[1]}) for color {target_color}")
                    last_log_time = time.time()
                    
                    while self.running and not colors_close(get_pixel_color(*monitor_pos), target_color):
                        current_time = time.time()
                        if current_time - last_log_time >= 1.0:  # Log once per second
                            current_color = get_pixel_color(*monitor_pos)
                            self.log_to_monitor(f"⏳ Waiting ({monitor_pos[0]}, {monitor_pos[1]}) with color {current_color} to become {target_color}")
                            last_log_time = current_time
                        time.sleep(0.1)
                    
                    if self.running:
                        final_color = get_pixel_color(*monitor_pos)
                        self.log_to_monitor(f"🎯 Color change detected! Pixel is now {final_color}")
                else:
                    # Monitor for any color change
                    initial_color = get_pixel_color(*monitor_pos)
                    self.log_to_monitor(f"👁️ Monitoring pixel ({monitor_pos[0]}, {monitor_pos[1]}) for any color change (initial: {initial_color})")
                    last_log_time = time.time()
                    
                    while self.running and get_pixel_color(*monitor_pos) == initial_color:
                        current_time = time.time()
                        if current_time - last_log_time >= 1.0:  # Log once per second
                            self.log_to_monitor(f"⏳ Waiting ({monitor_pos[0]}, {monitor_pos[1]}) with color {initial_color} for any change")
                            last_log_time = current_time
                        time.sleep(0.1)
                    
                    if self.running:
                        final_color = get_pixel_color(*monitor_pos)
                        self.log_to_monitor(f"🎯 Color change detected! Pixel changed from {initial_color} to {final_color}")
